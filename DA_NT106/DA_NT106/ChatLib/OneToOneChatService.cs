...
namespace ChatLib
{
    ...
    public class OneToOneChatService
    {
        ...
        private Dictionary<string, IOneToOneCallback> loggedInUsers = new Dictionary<string, IOneToOneCallback>();
        #endregion Fields
        ...
        public bool AddMeToServer(string userName)
        {
            ...
            IOneToOneCallback callback = OperationContext.Current.GetCallbackChannel<IOneToOneCallback>();
        ...
        private void SendMessageToParticularClient(SingleChatMessage chatmessage, string sender, string intendedReceiver)
        {
            ...
            foreach (string key in callbackKeys)
            {
                if (key == intendedReceiver || key == sender)
                {
                    try
                    {
                        IOneToOneCallback callback = loggedInUsers[key];

    ...
    public partial class Server : Form
    {
        ...
        private void serverBtn_Click(object sender, EventArgs e)
        {
            ...
            while (true)
            {
                ...
                int bytesRead = c.Receive(buffer);
                ...
                if (req == 1) // sign in
                {
                    ...
                    string uid = Encoding.UTF8.GetString(buffer, 0, bytesRead);

                    bytesRead = c.Receive(buffer);
                    int c1 = int.Parse(Encoding.UTF8.GetString(buffer, 0, bytesRead));

                    bytesRead = c.Receive(buffer);
                    int z = int.Parse(Encoding.UTF8.GetString(buffer, 0, bytesRead));
                    ...
                    using (var csv = new CsvReader(reader, System.Globalization.CultureInfo.CurrentCulture))
                    {
                        ...
                        while (csv.Read())
                        {
                            ...
                            if (currUid == uid)
                            {
                                ...
                                int hsh = int.Parse(hex, System.Globalization.NumberStyles.HexNumber);
                                int c2 = hsh % 10;
